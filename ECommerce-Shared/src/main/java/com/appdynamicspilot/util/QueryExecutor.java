...
import com.appdynamicspilot.jdbc.MConnection;
...
public class QueryExecutor {
    ...
    public void executeSimplePS(long sleep){
        ...
        if(con!=null){
            try{
                ...
                String query = "insert into OrderRequest ( item_id, notes ) values (  '3',  'Audacity Of Hope of out of stock' )";
                pstat = con.prepareStatement(query);
                pstat.setLong(1, 3l);
                pstat.setString(2,"Audacity Of Hope of out of stock");
                long startT = System.currentTimeMillis();
    ...
    public void executeQuery(long sleep,String orderIds,String notes){
        SLEEP_TIME = sleep;
        Connection con = getMockDBConnection();
        if(con!=null){
            try{
                PreparedStatement pstat;
                String query = "insert into inbound_inventory ( orderIds, notes ) values (  ?, ? )";
                pstat = con.prepareStatement(query);
                pstat.setString(1,orderIds);
                pstat.setString(2,notes);
                long startT = System.currentTimeMillis();
                ResultSet rset = pstat.executeQuery();
                System.out.println("mock query took = "+(System.currentTimeMillis() - startT));
                if(rset.next()){
